          ...
          {items.map((item) => (
            <SidebarMenuItem key={item.title}>
              <SidebarMenuButton
                ...
                tooltip={item.title}
                className="h-9  px-3 py-2 text-[15px] tracking-tight font-medium border border-transparent data-[active=true]:border-border data-[active=true]:shadow-[0px_1px_1px_0px_rgba(44,54,53,0.03),inset_0px_0px_0px_2px_white]"
              >
                {item.url ? (
                  <Link href={item.url} className="flex flex-1 gap-4 items-center justify-start">
                    <item.icon />
...
//   const [voiceDialogOpen, setVoiceDialogOpen] = useState(false);
  ...
  const othersMenuItems: MenuItem[] = [
    ...
    },
    {
      ...
      url: "mailto:chaturvediratnesh16.com",
      icon: Headphones,
    },
  ];
